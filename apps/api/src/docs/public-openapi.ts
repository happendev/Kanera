...
const queryParam = (name: string, schema: Schema, description?: string, required = false): Schema => ({
  name,
  in: "query",
  required,
  description,
...
\`GET /boards\`; a workspace-scoped key created there can still open its pinned workspace. Creating
another board is rejected. Imports started from the Kanera app populate the
existing sole board instead of creating a second one.

\`GET /workspaces\` therefore returns only workspaces the credential can access at workspace scope. It does not return the parent workspaces of boards shared with a cross-organisation guest. Use \`GET /boards\` for complete board discovery, including board-only guest access.
...
# Include standalone and board-only cross-organisation guest access.
curl "$KANERA_PUBLIC_API_URL/api/v1/boards" \\
  -H "Authorization: Bearer $KANERA_API_KEY"
...
export const publicOpenApiDocument: Record<string, unknown> = {
  ...
  tags: [
    ...
    { name: "Workspaces", description: "Discover workspace-level access and manage workspace membership and boards. Board-only cross-organisation guest access is returned separately by `GET /boards`." },
    ...
    { name: "Search", description: "Search accessible live Kanera content. Agent search returns one bounded, typed stream with canonical source links." },
    {
      name: "External Links",
  ...
  components: {
    ...
    schemas: {
      ...
      Note: {
        type: "object",
        required: ["id", "workspaceId", "boardId", "parentNoteId", "scope", "ownerId", "lastEditedById", "lastEditedByName", "title", "content", "position", "createdAt", "lastEditedAt", "updatedAt"],
        properties: {
          id: uuid,
          workspaceId: uuid,
          boardId: nullable(uuid),
          parentNoteId: nullable(uuid),
          scope: { type: "string", enum: ["personal", "team"], description: "Personal notes are visible only to their owner." },
          ownerId: uuid,
          lastEditedById: uuid,
          lastEditedByName: { type: "string" },
          lastEditedByAvatarUrl: nullable({ type: "string", format: "uri" }),
          title: { type: "string" },
          content: { type: "string", description: "Markdown content. Ordinary and Kanera-internal links can be added here." },
          icon: nullable({ type: "string" }),
          color: nullable({ type: "string" }),
          position,
          editingUserId: nullable(uuid),
          editingExpiresAt: nullable(dateTime),
          createdAt: dateTime,
      ...
      NoteAttachment: {
        type: "object",
        required: ["id", "noteId", "fileName", "mimeType", "byteSize", "url", "source", "createdAt", "uploadedById", "uploadedByName"],
        properties: {
          id: uuid,
          noteId: uuid,
          fileName: { type: "string" },
          ...
          url: { type: "string", format: "uri" },
          source: { type: "string", enum: ["description", "attachment"] },
          createdAt: dateTime,
          uploadedById: uuid,
          uploadedByName: { type: "string" },
          uploadedByAvatarUrl: nullable({ type: "string", format: "uri" }),
        },
        additionalProperties: true,
      },
      NoteMoveResult: {
        type: "object",
        required: ["id", "parentNoteId", "position"],
        properties: {
          id: uuid,
          parentNoteId: nullable(uuid),
          position,
      ...
      NoteBacklinks: {
        type: "object",
        required: ["backlinks"],
        properties: { backlinks: { type: "array", items: { type: "object", additionalProperties: true } } },
        additionalProperties: false,
      },
      Comment: {
        type: "object",
      ...
      AgentSearchQueryBody: zodSchema(dto.agentSearchQueryBody),
      AgentSearchCardResult: {
        type: "object",
        required: ["type", "id", "matchContext", "workspaceId", "workspaceName", "boardId", "boardName", "cardId", "cardKey", "cardTitle", "listName", "url"],
        properties: {
          type: { type: "string", enum: ["card"] },
          id: uuid,
          matchContext: { type: "string" },
          workspaceId: uuid,
      ...
      AgentSearchNoteResult: {
        type: "object",
        required: ["type", "id", "title", "matchContext", "workspaceId", "workspaceName", "boardId", "boardName", "url"],
        properties: {
          ...
          workspaceName: { type: "string" },
          boardId: nullable(uuid),
          boardName: nullable({ type: "string" }),
          url: { type: "string", format: "uri" },
        },
        additionalProperties: false,
      },
      AgentSearchAttachmentResult: {
        type: "object",
        required: ["type", "id", "fileName", "matchContext", "workspaceId", "workspaceName", "boardId", "boardName", "cardId", "cardKey", "cardTitle", "listName", "url"],
        properties: {
      ...
      AgentSearchResult: {
        oneOf: [ref("AgentSearchCardResult"), ref("AgentSearchCommentResult"), ref("AgentSearchNoteResult"), ref("AgentSearchAttachmentResult")],
        discriminator: { propertyName: "type" },
      },
      AgentSearchResponse: {
        type: "object",
        required: ["query", "results"],
        properties: { query: { type: "string" }, results: arrayOf(ref("AgentSearchResult")) },
        additionalProperties: false,
      },
      GroupedSearchResponse: {
        type: "object",
        required: ["query", "cards", "notes", "comments", "attachments"],
        properties: {
          query: { type: "string" },
      ...
      AgentWorkHistoryQueryBody: zodSchema(dto.agentWorkHistoryQueryBody),
      AgentWorkSources: {
        type: "object",
        required: ["boards", "lists", "labels", "people"],
        properties: {
          boards: arrayOf({
            type: "object",
            required: ["id", "name", "url", "workspaceId", "workspaceName", "organisationId", "organisationName"],
            properties: { id: uuid, name: { type: "string" }, url: { type: "string", format: "uri" }, workspaceId: uuid, workspaceName: { type: "string" }, organisationId: uuid, organisationName: { type: "string" } },
            additionalProperties: false,
          }),
          lists: arrayOf({ type: "object", required: ["id", "workspaceId", "name"], properties: { id: uuid, workspaceId: uuid, name: { type: "string" } }, additionalProperties: false }),
          labels: arrayOf({ type: "object", required: ["id", "workspaceId", "name", "color"], properties: { id: uuid, workspaceId: uuid, name: { type: "string" }, color: nullable({ type: "string" }) }, additionalProperties: false }),
          people: arrayOf({ type: "object", required: ["id", "displayName"], properties: { id: uuid, displayName: { type: "string" } }, additionalProperties: false }),
        },
        additionalProperties: false,
      },
      AgentWorkHistoryEvent: {
        type: "object",
        required: ["id", "type", "at", "card", "boardId", "listId"],
        properties: {
          id: { type: "string" },
          type: { type: "string", enum: ["created", "moved", "completed", "checklistItemCompleted"] },
          at: dateTime,
          card: { type: "object", required: ["id", "boardId", "listId", "title", "url"], properties: { id: uuid, boardId: uuid, listId: uuid, title: { type: "string" }, url: { type: "string", format: "uri" } }, additionalProperties: true },
          boardId: uuid,
          listId: uuid,
          actorUserId: nullable(uuid),
          actorName: { type: "string" },
          actorAvatarUrl: nullable({ type: "string", format: "uri" }),
          listPath: arrayOf(uuid),
          itemId: uuid,
          text: { type: "string" },
          checklistId: uuid,
          checklistTitle: { type: "string" },
          completedByUserId: nullable(uuid),
          completedByName: { type: "string" },
          completedByAvatarUrl: nullable({ type: "string", format: "uri" }),
        },
        additionalProperties: false,
      },
      AgentWorkHistoryPage: {
        ...
        properties: {
          actor: { type: "object", required: ["userId", "displayName"], properties: { userId: uuid, displayName: { type: "string" } }, additionalProperties: false },
          range: { type: "object", required: ["from", "to", "timeZone"], properties: { from: dateTime, to: dateTime, timeZone: { type: "string" } }, additionalProperties: false },
          summary: { type: "object", required: ["created", "moved", "completed", "checklistItemCompleted", "cardsTouched", "totalEvents"], properties: { created: { type: "integer", minimum: 0 }, moved: { type: "integer", minimum: 0 }, completed: { type: "integer", minimum: 0 }, checklistItemCompleted: { type: "integer", minimum: 0 }, cardsTouched: { type: "integer", minimum: 0 }, totalEvents: { type: "integer", minimum: 0 } }, additionalProperties: false },
          events: arrayOf(ref("AgentWorkHistoryEvent")),
          sources: ref("AgentWorkSources"),
          nextCursor: nullable({ type: "string" }),
        },
        additionalProperties: false,
      },
      AgentCurrentWorkPage: ref("WorkCardsPage"),
      WorkCardsPage: {
        type: "object",
        required: ["cards", "separators", "separatorWorkspaceIds", "checklistItems", "totals", "sources", "nextCursor"],
        properties: {
          cards: arrayOf({ type: "object", required: ["id", "boardId", "listId", "title", "url"], properties: { id: uuid, boardId: uuid, listId: uuid, title: { type: "string" }, url: { type: "string", format: "uri" } }, additionalProperties: true }),
          separators: arrayOf({ type: "object", additionalProperties: true }),
          separatorWorkspaceIds: arrayOf(uuid),
          checklistItems: arrayOf({ type: "object", required: ["itemId", "cardId", "boardId", "listId", "url"], properties: { itemId: uuid, cardId: uuid, boardId: uuid, listId: uuid, url: { type: "string", format: "uri" } }, additionalProperties: true }),
      ...
      AccessibleBoard: {
        ...
        properties: {
          ...
          workspaceId: uuid,
          workspaceName: { type: "string" },
          workspaceIcon: nullable({ type: "string" }),
          workspaceAccentColor: nullable({ type: "string" }),
          workspaceKind: { type: "string", enum: ["standard", "board"] },
          clientId: uuid,
          ...
          assignedItemsOnly: { type: "boolean" },
          canAccessWorkspace: { type: "boolean" },
      ...
      WorkDonePage: {
        type: "object",
        required: ["events"],
        properties: { events: arrayOf({ type: "object", additionalProperties: true }) },
        additionalProperties: false,
      },
      WorkDoneSummary: {
        type: "object",
      ...
      UpdateAutomationBody: zodSchema(dto.updateAutomationBody),
      SetAutomationActionsBody: zodSchema(dto.setAutomationActionsBody),
      MoveAutomationBody: zodSchema(dto.moveAutomationBody),
      Automation: {
        type: "object",
        required: ["id", "workspaceId", "enabled", "position", "triggerType", "actions", "runStats", "createdAt", "updatedAt"],
        properties: {
          id: uuid,
          workspaceId: uuid,
          enabled: { type: "boolean" },
          position,
          triggerType: zodSchema(dto.automationTriggerType),
          ...
          runStats: nullable({ type: "object", additionalProperties: true }),
          createdAt: dateTime,
          updatedAt: dateTime,
        },
        additionalProperties: true,
      ...
      DuplicateNoteBody: zodSchema(dto.duplicateNoteBody),
  ...
  paths: {
    ...
    "/workspaces/{wsId}/automations": {
      get: operation({
        ...
        description: "Returns active rules in execution order with their ordered actions and lifetime run statistics. Requires workspace-admin authority.",
        operationId: "listAutomations",
        parameters: [idParam("wsId", "Workspace id.")],
        responses: authedResponses({ "200": ok(arrayOf(ref("Automation"))) }),
      }),
      post: operation({
        tags: ["Automations"],
        ...
        responses: authedResponses({ "201": created(ref("Automation")) }),
      }),
    },
    "/automations/{id}": {
      patch: operation({
        ...
        description: "Updates trigger settings, enabled state, and/or replaces the ordered action list atomically. Replacing actions with an empty list disables the rule. Requires workspace-admin authority and a write-capable credential.",
        operationId: "updateAutomation",
        parameters: [idParam()],
        requestBody: jsonBody(ref("UpdateAutomationBody")),
        responses: authedResponses({ "200": ok(ref("Automation")) }),
      }),
      delete: operation({
        tags: ["Automations"],
        summary: "Delete an automation",
        description: "Archives the rule and stops future executions without undoing prior effects. Requires workspace-admin authority and a write-capable credential.",
        operationId: "deleteAutomation",
        parameters: [idParam()],
        responses: authedResponses({ "204": noContent }),
      }),
    },
    "/automations/{id}/actions": pathItem("put", operation({
      tags: ["Automations"],
      summary: "Replace automation actions",
      operationId: "setAutomationActions",
      parameters: [idParam()],
      requestBody: jsonBody(ref("SetAutomationActionsBody")),
      responses: authedResponses({ "200": ok(ref("Automation")) }),
    })),
    "/automations/{id}/move": pathItem("post", operation({
      tags: ["Automations"],
      summary: "Reorder an automation",
      operationId: "moveAutomation",
      parameters: [idParam()],
      requestBody: jsonBody(ref("MoveAutomationBody")),
      responses: authedResponses({ "200": ok({ type: "object", required: ["id", "position"], properties: { id: uuid, position } }) }),
    })),
    "/automations/{id}/executions": pathItem("get", operation({
      tags: ["Automations"],
      summary: "List automation executions",
      description: "Returns retained execution outcomes newest first. Requires workspace-admin authority.",
    ...
    "/boards/{id}/completed": pathItem("get", operation({
      tags: ["Boards"],
      summary: "List completed board cards",
      operationId: "listCompletedBoardCards",
      parameters: [
        idParam(),
        queryParam("from", dateTime),
        queryParam("to", dateTime),
        queryParam("listId", uuid),
      ...
      responses: authedResponses({ "200": ok(ref("CompletedCardsPage")) }),
    })),
    "/boards/{id}/work-done": pathItem("get", operation({
      tags: ["Boards"],
      summary: "List board work activity",
      description: "Lists created, moved, completed, and checklist-completion events within the requested time window.",
      operationId: "listBoardWorkDone",
      parameters: [
        idParam(),
        queryParam("from", dateTime, "Inclusive window start.", true),
        queryParam("to", dateTime, "Inclusive window end.", true),
        queryParam("timeZone", { type: "string", default: "UTC" }),
        queryParam("q", { type: "string", minLength: 1, maxLength: 200 }),
        queryParam("listIds", arrayOf(uuid)),
        queryParam("labelIds", arrayOf(uuid)),
        queryParam("actorIds", arrayOf(uuid)),
      ],
      responses: authedResponses({ "200": ok(ref("WorkDonePage")) }),
    })),
    "/boards/{id}/work-done/summary": pathItem("get", operation({
      tags: ["Boards"],
      summary: "Summarize board work activity",
      operationId: "summarizeBoardWorkDone",
      parameters: [
        idParam(),
        queryParam("from", dateTime, "Inclusive window start.", true),
        queryParam("to", dateTime, "Inclusive window end.", true),
      ...
      responses: authedResponses({ "200": ok(ref("WorkDoneSummary")) }),
    })),
    ...
    "/workspaces/{id}/guests": pathItem("get", operation({
      ...
      description: "Lists accepted cross-organisation guests and pending invitations. For a standalone board, use the workspace id returned alongside the board by `GET /boards` or standalone creation.",
    ...
    "/notes/{id}": {
      ...
      patch: operation({
        ...
        description: "Edit title, Markdown content, icon, or color. Add external or Kanera-internal links in content. To embed an attachment, upload it first and then add its returned URL to content.",
        operationId: "updateNote",
        parameters: [idParam()],
        requestBody: jsonBody(ref("UpdateNoteBody")),
        responses: authedResponses({ "200": ok(ref("Note")) }),
      }),
    },
    "/notes/{id}/duplicate": pathItem("post", operation({
      tags: ["Notes"],
      summary: "Duplicate a note",
      description: "Copies one note's title, content, icon, color, and internal-link relationships. Descendant notes and binary attachments are not copied.",
      operationId: "duplicateNote",
      parameters: [idParam()],
      requestBody: jsonBody(ref("DuplicateNoteBody")),
      responses: authedResponses({ "201": created(ref("Note")) }),
    })),
    "/notes/{id}/move": pathItem("patch", operation({
      tags: ["Notes"],
      summary: "Move a note in its hierarchy",
      description: "Reparents or reorders a note within the same workspace/board and personal/team collection.",
      operationId: "moveNote",
      parameters: [idParam()],
      requestBody: jsonBody(ref("MoveNoteBody")),
      responses: authedResponses({ "200": ok(ref("NoteMoveResult")) }),
    })),
    "/notes/{id}/backlinks": pathItem("get", operation({ tags: ["Notes"], summary: "List note backlinks", operationId: "listNoteBacklinks", parameters: [idParam()], responses: authedResponses({ "200": ok(ref("NoteBacklinks")) }) })),
    "/notes/{id}/attachments": {
      get: operation({ tags: ["Notes"], summary: "List note attachments", operationId: "listNoteAttachments", parameters: [idParam()], responses: authedResponses({ "200": ok(arrayOf(ref("NoteAttachment"))) }) }),
      post: operation({
        tags: ["Notes"],
        summary: "Upload a note attachment",
        description: "Uploads one file. Use source=description for inline media, then add the returned URL to the note's Markdown content; use source=attachment for a regular file attachment.",
        operationId: "uploadNoteAttachment",
        parameters: [idParam(), queryParam("source", { type: "string", enum: ["description", "attachment"], default: "attachment" })],
        requestBody: multipartBody("Upload a single note attachment file."),
        responses: authedResponses({ "201": created(ref("NoteAttachment")) }),
      }),
    },
    ...
    "/search": {
      get: operation({
        tags: ["Search"],
        summary: "Search accessible Kanera content in grouped collections",
        description: "Searches accessible cards, notes, comments, and attachment filenames. The limit applies separately to each result collection; use POST /search/query for one relevance-ranked total limit and explicit result types.",
        operationId: "searchContent",
        parameters: [
          queryParam("q", { type: "string", minLength: 1, maxLength: 200 }, "Words, phrase, or card key to find.", true),
          queryParam("limit", { type: "integer", minimum: 1, maximum: 20 }, "Maximum rows in each entity collection."),
        ],
        responses: authedResponses({ "200": ok(ref("GroupedSearchResponse")) }),
      }),
      post: operation({
        tags: ["Search"],
        summary: "Search accessible Kanera content for an agent",
        description: "Returns one relevance-ranked result stream across cards, comments, notes, and attachment filenames. The total limit applies across all requested types. Every result identifies its type and includes accessible workspace/board/card context plus a canonical Kanera URL.",
        operationId: "queryAgentSearch",
        requestBody: jsonBody(ref("AgentSearchQueryBody")),
        responses: authedResponses({ "200": ok(ref("AgentSearchResponse")) }),
      }),
    },
    "/work/history/query": pathItem("post", operation({
      tags: ["Activity"],
      summary: "Query one person's cross-board work history",
      description: "Returns only the requested actor's created, moved, completed, and checklist-item-completed events across accessible workspace, standalone, and guest boards. Omit userId for the connected user. Another user must be visible inside the selected scope. Use either a calendar preset or an exact from/to range; results include full-range counts, source metadata, canonical card links, and cursor pagination.",
      operationId: "queryWorkHistory",
      requestBody: jsonBody(ref("AgentWorkHistoryQueryBody")),
