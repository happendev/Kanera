...
function normalizeIp(ip: string | undefined) {
  ...
  if (trimmed.startsWith("::ffff:") && isIP(trimmed.slice("::ffff:".length)) === 4) {
...
function parseIp(ip: string) {
  ...
  if (family === 4) return { family: 4 as const, value: parseIpv4(normalized)! };
  if (family === 6) return { family: 6 as const, value: parseIpv6(normalized)! };
  return null;
}
...
export function resolveClientIp(input: {
  ...
  if (cfConnectingIp && isIP(cfConnectingIp) && isCloudflarePeer(input.remoteAddress)) {
    return cfConnectingIp;
  }

  // Fastify's req.ip already accounts for trustProxy when that is enabled.
