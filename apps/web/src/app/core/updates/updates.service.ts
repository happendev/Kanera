...
export class UpdatesService {
    ...
    this.swUpdate.versionUpdates
      ...
      .subscribe((evt) => {
        if (evt.type === "VERSION_DETECTED") {
          console.log("[SW] New version available:", evt.version.hash);
          return;
        }

        console.log("[SW] New version ready:", evt.currentVersion.hash, "→", evt.latestVersion.hash);
        this.updateAvailable.set(true);
      });
  }

  private listenForUnrecoverable() {
    this.swUpdate.unrecoverable.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((evt) => {
      console.warn("[SW] Unrecoverable state:", evt.reason, "— reloading");
  ...
  private setupVisibilityPolling() {
  ...
  async checkForUpdate(): Promise<void> {
    ...
    try {
      ...
      console.warn("[SW] checkForUpdate failed:", err);
    }
  }
