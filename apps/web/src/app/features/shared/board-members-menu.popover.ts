...
export type BoardAccessMemberRow = {
  boardId: string; userId: string; role: BoardRole; pinned: boolean; addedAt: string | Date;
          ...
          @for (member of localMembers(); track member.userId) {
            <div class="bmp-row">
              ...
              <div class="bmp-actions">
                ...
                } @else { <span class="bmp-role">{{ roleLabel(member.role) }}</span> }
              </div>
            </div>
          ...
          <section class="bmp-section bmp-guests">
            ...
            @for (guest of guests(); track guest.userId) {
              <div class="bmp-row">
                ...
                <div class="bmp-actions"><span class="bmp-role">{{ roleLabel(guest.role) }}</span></div>
              </div>
            }
            <p class="bmp-hint">Guests are managed in workspace settings.</p>
          </section>
        }
      }
    </div>
...
export class BoardMembersMenu implements OnInit, AfterViewInit, OnDestroy {
  ...
  readonly accessMembers = signal<BoardAccessMemberRow[]>([]); readonly roster = signal<WorkspaceMemberRow[]>([]); readonly loading = signal(false); readonly busy = signal(false); readonly error = signal<string | null>(null); readonly addUserId = signal(""); readonly addRole = signal<BoardRole>("observer");
  ...
  async addMember(){const userId=this.addUserId();if(!userId||this.busy())return;this.busy.set(true);this.error.set(null);try{await this.api.post(`/boards/${this.boardId()}/members`,{userId,role:this.addRole()});this.addUserId.set("");await this.reload(false)}catch(e){this.error.set(errorMessage(e))}finally{this.busy.set(false)}}
