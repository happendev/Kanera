...
export interface BoardExportPayload {
  ...
  /** Overall aggregates across the deduplicated exported cards, matching the table-view total row. */
...
/**
 * A built sheet as write-excel-file's own cell matrix.
 *
 * The builders above stay free of any write-excel-file value shape so they remain unit-testable
 * without the library; this is the single place that translation happens, shared by every view that
 * exports a workbook so the two cannot render the same sheet differently.
 */
export function styledSheetData(sheet: WorkbookSheet): SheetData {
