...
export interface GroupingContext {
  ...
  now?: Date;
  /**
   * The source chain, supplied only by cross-board surfaces. Absent on a board, which is why the
   * board/workspace/organisation modes fall back to no grouping rather than one "Unknown" block.
   */
...
export function groupCards(
  ...
  switch (mode) {
    ...
    case "workspace":
    case "organisation":
      return groupBySource(cards, mode, ctx, sortMode);
    case "none":
    default:
...
/**
 * Group by where the card comes from: its board, that board's workspace, or that workspace's
 * organisation. One walk of the same chain covers all three, since every level is reached from
 * `card.boardId`.
 *
 * Buckets follow catalog order, not first-seen order — the catalog is already sorted the way the
 * sidebar and the scope picker present these, and a report that reorders its sections depending on
 * which board happened to hold the first matching card is not a report anyone can compare week to
 * week. Cards whose source is missing from the catalog (access revoked mid-session, a board deleted
 * between the query and the render) land in one trailing bucket rather than vanishing.
 */
function groupBySource(
  cards: AnyCard[],
  mode: "board" | "workspace" | "organisation",
  ctx: GroupingContext,
  sortMode: SortBy,
): CardGroup[] {
  const boards = ctx.boards;
  if (!boards?.length) return groupByNoGrouping(cards, sortMode);
  const boardById = new Map(boards.map((board) => [board.id, board]));
  const workspaceById = new Map((ctx.workspaces ?? []).map((workspace) => [workspace.id, workspace]));

  const byKey = new Map<string, AnyCard[]>();
  const unknown: AnyCard[] = [];
  for (const card of cards) {
    const board = boardById.get(card.boardId);
    const workspace = board ? workspaceById.get(board.workspaceId) : undefined;
    const key = mode === "board"
      ? board?.id
      : mode === "workspace"
        ? workspace?.id
        : workspace?.organisationId;
    if (!key) unknown.push(card);
    else appendGroupCard(byKey, key, card);
  }

  const ordered: { id: string; label: string; icon: string; color: string | null }[] =
    mode === "board"
      ? boards.map((board) => ({
        id: board.id,
        label: board.name,
  ...
  const groups: CardGroup[] = ordered
    .filter((entry) => byKey.has(entry.id))
    .map((entry) => ({
      key: `${mode}:${entry.id}`,
      label: entry.label,
      icon: entry.icon,
      ...
      cards: sortGroupCards(byKey.get(entry.id) ?? [], sortMode),
    }));

  if (unknown.length) {
    groups.push({
      key: `${mode}:${NULL_GROUP_KEY}`,
      label: mode === "board" ? "Unknown board" : mode === "workspace" ? "Unknown workspace" : "Unknown organisation",
      icon: "help-circle",
      color: "gray",
      acceptsDrop: false,
      meta: {},
      cards: sortGroupCards(unknown, sortMode),
    });
  }
  return groups;
}
