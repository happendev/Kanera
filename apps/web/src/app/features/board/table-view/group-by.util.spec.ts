...
import type { AnyCard, AnyCustomField, AnyLabel, AnyList, AnyMember } from "./table-view.types";
...
describe("groupCards", () => {
  ...
  // Cross-board dimensions, used by Global Work. All three walk the same board → workspace →
  // organisation chain from `card.boardId`.
  describe("source dimensions", () => {
    const sourceCtx = {
      ...baseCtx,
      boards: [
        { id: "b1", workspaceId: "w1", name: "Delivery", icon: "rocket", iconColor: "blue" },
        { id: "b2", workspaceId: "w1", name: "Support", icon: null, iconColor: null },
        { id: "b3", workspaceId: "w2", name: "Internal", icon: null, iconColor: null },
      ],
      workspaces: [
        { id: "w1", organisationId: "o1", name: "Acme", icon: null, accentColor: "green" },
        { id: "w2", organisationId: "o2", name: "Ourselves", icon: null, accentColor: null },
      ],
      organisations: [{ id: "o1", name: "Acme Ltd" }, { id: "o2", name: "Us" }],
    };
    const sourceCards = [
      card({ id: "a", boardId: "b2", position: "2000" }),
      card({ id: "b", boardId: "b1", position: "1000" }),
      card({ id: "c", boardId: "b3", position: "1000" }),
    ];

    it("groups by board in catalog order, not first-seen order", () => {
      const groups = groupCards(sourceCards, "board", "position", sourceCtx);

      expect(groups.map((group) => group.key)).toEqual(["board:b1", "board:b2", "board:b3"]);
      expect(groups.map((group) => group.label)).toEqual(["Delivery", "Support", "Internal"]);
      expect(groups[0]!.icon).toBe("rocket");
      expect(groups[0]!.color).toBe("blue");
      // Falls back to the generic board glyph rather than rendering nothing.
      expect(groups[1]!.icon).toBe("layout-kanban");
    });

    it("rolls boards up to their workspace and organisation", () => {
      expect(groupCards(sourceCards, "workspace", "position", sourceCtx).map((group) => ({
        label: group.label,
        cards: group.cards.map((c) => c.id),
      }))).toEqual([
        { label: "Acme", cards: ["b", "a"] },
        { label: "Ourselves", cards: ["c"] },
      ]);

      expect(groupCards(sourceCards, "organisation", "position", sourceCtx).map((group) => ({
        label: group.label,
        cards: group.cards.map((c) => c.id),
      }))).toEqual([
        { label: "Acme Ltd", cards: ["b", "a"] },
        { label: "Us", cards: ["c"] },
      ]);
    });

    // Access can be revoked, or a board deleted, between the query and the render. Dropping those
    // rows would silently shorten the sheet and every total on it.
    it("collects cards whose source is missing from the catalog", () => {
      const groups = groupCards(
        [...sourceCards, card({ id: "d", boardId: "gone" })],
        "board",
        "position",
        sourceCtx,
      );

      expect(groups.at(-1)!.label).toBe("Unknown board");
      expect(groups.at(-1)!.cards.map((c) => c.id)).toEqual(["d"]);
      expect(groups.flatMap((group) => group.cards)).toHaveLength(4);
    });

    // A board never supplies the chain, so the axis is not offered there — but a stored preference
    // could still name it, and one block called "Unknown board" would be worse than no grouping.
    it("falls back to no grouping without a source catalog", () => {
      const groups = groupCards(sourceCards, "board", "position", baseCtx);
