...
describe("FilterBarComponent", () => {
  ...
  it("keeps the Filter label by default and hides only it in compact mode", () => {
    const fixture = makeFixture(EMPTY);
    expect((fixture.nativeElement.querySelector(".fb-btn") as HTMLElement).textContent).toContain("Filter");

    fixture.componentRef.setInput("compact", true);
    fixture.detectChanges();
    expect((fixture.nativeElement.querySelector(".fb-btn") as HTMLElement).textContent).not.toContain("Filter");
    expect(fixture.nativeElement.querySelector(".fb-btn i.ti-filter")).not.toBeNull();
  });
  ...
  it("sections the menu so the dimensions are scannable rather than one flat list", () => {
    const fixture = makeFixture(EMPTY, { showCompleted: true, showArchived: true });

    clickButton(fixture, "Filter");
    const sections = Array.from((fixture.nativeElement as HTMLElement).querySelectorAll(".fb-section-label"))
      .map((node) => node.textContent?.trim());
    expect(sections).toEqual(["Quick filters", "Card details", "Card status"]);
  });

  it("omits Hide completed unless the page opts in", () => {
    // A board's default view has no such toggle, so it must not appear unless requested.
    const fixture = makeFixture(EMPTY, { showCompleted: true, showArchived: true });
    clickButton(fixture, "Filter");
    expect((fixture.nativeElement as HTMLElement).textContent).not.toContain("Hide completed");
  });

  it("emits Hide completed and counts it as an active filter", () => {
    const emissions: boolean[] = [];
    const fixture = makeFixture(EMPTY, { showHideCompleted: true });
    fixture.componentInstance.hideCompletedChange.subscribe((hide: boolean) => emissions.push(hide));

    clickButton(fixture, "Filter");
    clickButton(fixture, "Hide completed");
    expect(emissions).toEqual([true]);

    fixture.componentRef.setInput("hideCompleted", true);
    fixture.detectChanges();
    expect(fixture.componentInstance.activeCount()).toBe(1);
    expect(fixture.componentInstance.anyActive()).toBe(true);
  });

  it("groups option rows by their group so same-named options across workspaces are distinguishable", () => {
    const fixture = makeFixture(EMPTY, {
      lists: [
        { id: "li1", name: "Doing", icon: "list", group: "Marketing" },
        { id: "li2", name: "Doing", icon: "list", group: "Delivery" },
      ],
    });

    clickButton(fixture, "Filter");
    clickButton(fixture, "Lists");

    const host = fixture.nativeElement as HTMLElement;
    const groups = Array.from(host.querySelectorAll(".fb-group")).map((node) => node.textContent?.trim());
    expect(groups).toEqual(["Marketing", "Delivery"]);
    // Grouped rows are indented under their heading; without that the list still scans as flat.
    expect(Array.from(host.querySelectorAll(".fb-list .fb-row")).every((row) =>
      row.classList.contains("fb-indent")
    )).toBe(true);
  });

  it("labels an active custom-field condition with its workspace when one is supplied", () => {
    const fixture = makeFixture(
      { ...EMPTY, cfConditions: [{ fieldId: "f1", op: "isAnyOf", ids: ["o1"] }] },
      { customFields: [selectField()], customFieldGroups: { f1: "Marketing" } },
    );

    clickButton(fixture, "Filter");
    clickButton(fixture, "Custom fields");
    const condition = (fixture.nativeElement as HTMLElement).querySelector(".fb-row-static");
    expect(condition!.textContent!.replace(/\s+/g, " ")).toContain("Marketing");
  });
