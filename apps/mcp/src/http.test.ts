...
void test("HTTP MCP endpoint completes protocol initialization with a Kanera API key", async () => {
  await withHttpServer(async (baseUrl) => {
    ...
    assert.equal(payload.result?.serverInfo?.description, "Read and manage Kanera workspaces, standalone boards, lists, cards, assigned work, notes, comments, labels, custom fields, and activity.");
